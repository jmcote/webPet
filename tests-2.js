QUnit.test("Cube test", function (assert) {
    var daCube = new Cube(4,20);
    var result = daCube.length;
    assert.deepEqual(result, 4, "Success - l = 4");
    result = daCube.surfaceArea;
    assert.deepEqual(result, 96, "Success - sa = 96");
    result = daCube.volume;
    assert.deepEqual(result, 64, "Success - vol = 64");
});
...
QUnit.test("Test the roll over behavior.", function (assert) {
    ...
    myPet = new Pet("test", "Cat");
    var result = myPet.rollOver();
    result = myPet.catTriedRoll;
    assert.deepEqual(result, true, "Success - cats don't roll over (unless they want to)!");
    result = myPet.rollOver();
    result = myPet.rollOver();
    result = myPet.rollOver();
    result = myPet.rollOver();
    result = myPet.stats.rollAttempts;
    assert.deepEqual(result, 5, "Success - a cat won't roll over if it's hungry");
    myPet.hunger = 0;
    myPet.sleepy = 5;
    result = myPet.rollOver();
